...
const { data: fetchedCategory, pending, error, refresh: refreshCategory } = await useLazyAsyncData(
  ...
  async () => {
    try {
      ...
      if (props.categoryId) {
        const categories = await $fetch(`${baseURL}/wp-json/api/v1/categories`)
      ...
      const products = await $fetch(`${baseURL}/wp-json/api/v1/products/category/${categoryInfo.id}?per_page=100`)
      
      // Récupérer les sous-catégories
      const allCategories = await $fetch(`${baseURL}/wp-json/api/v1/categories`)
